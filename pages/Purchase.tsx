...
import { ShoppingBag, Plus, Phone, Mail, FileText, Download, Edit2, Trash2, X, Save, Minus, CheckCircle, Clock, Truck, CreditCard, Banknote, Search, AlertCircle, Loader2, Check, Calendar, ChevronDown, Store, ListFilter } from 'lucide-react';
...
const Purchase = () => {
  ...
  const filteredOrders = useMemo(() => {
    return purchaseOrders.filter(po => {
      ...
      const matchesPayment = filterPayment === 'ALL' || po.paymentType === filterPayment;
      
      let matchesDate = true;
      if (dateRange.start) matchesDate = matchesDate && po.date >= dateRange.start;
      if (dateRange.end) matchesDate = matchesDate && po.date <= dateRange.end;

      return matchesSearch && matchesStatus && matchesPayment && matchesDate;
    });
  }, [purchaseOrders, searchTerm, filterStatus, filterPayment, dateRange]);
             ...
             <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
                ...
                <div className="flex items-center gap-2 relative" ref={filterRef}>
                   ...
                   <div className="relative">
                      ...
                      <input 
                        className="h-10 pl-10 pr-4 bg-white border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-200 w-64 transition-all placeholder:text-slate-400" 
                   ...
                   <button 
                     ...
                     className={`h-10 px-3 flex items-center gap-2 border rounded-md text-sm font-medium transition-colors ${isFilterOpen || filterStatus !== 'ALL' || filterPayment !== 'ALL' || dateRange.start ? 'bg-slate-100 border-slate-300 text-slate-800' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                   >
                      <ListFilter size={16} />
                      <span className="hidden sm:inline">Filter</span>
                      {(filterStatus !== 'ALL' || filterPayment !== 'ALL' || dateRange.start) && (
                   ...
                   {/* Filter Popover */}
                   {isFilterOpen && (
                      <div className="absolute right-0 top-12 w-72 bg-white rounded-xl shadow-xl border border-slate-100 z-30 p-4 animate-in fade-in slide-in-from-top-2">
                         <div className="space-y-4">
                            <div>
                               <label className="text-xs font-semibold text-slate-500 uppercase mb-1 block">Status</label>
                               <select 
                                  className="w-full h-9 px-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                                  value={filterStatus}
                            ...
                            <div>
                               <label className="text-xs font-semibold text-slate-500 uppercase mb-1 block">Payment Type</label>
                               <select 
                                  className="w-full h-9 px-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                                  value={filterPayment}
                            ...
                            <div>
                               ...
                               <div className="grid grid-cols-2 gap-2">
                                  <input 
                                    type="date" 
                                    className="h-9 px-2 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-blue-500" 
                    ...
                    {filteredOrders.length === 0 && (
                      <tr>
                        <td colSpan={6} className="px-6 py-12 text-center text-slate-400">
                           ...
                           <p>No purchase orders found matching filters.</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
