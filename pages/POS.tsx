...
import { Search, Plus, Minus, RotateCcw, ShoppingCart, ScanLine, Image as ImageIcon, CheckCircle, AlertCircle, X, Banknote, QrCode, Lock, AlertTriangle, User, ChevronRight } from 'lucide-react';
...
const ProductCard: React.FC<ProductCardProps> = ({ product, onAdd, index }) => (
  <div 
  ...
  >
    ...
    <div className="p-3 flex flex-col flex-1">
      ...
      <div className="flex items-center gap-1.5 mt-1 text-xs text-slate-500">
         <span className="bg-slate-100 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider text-slate-600">{product.category.substring(0, 8)}</span>
...
const POS = () => {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<'CASH' | 'KBZ_PAY'>('CASH');
  ...
  // Updated Retail Categories
  const categories = ['All', 'Beverages', 'Snacks', 'Bakery', 'Dairy', 'Pantry', 'Household', 'Personal Care'];
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-slate-50 relative">
      ...
      {/* Payment Modal - Ultra Simple Selection */}
      {paymentModalOpen && (
            ...
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-in zoom-in-95 duration-200 scale-100">
               ...
               <div className="grid grid-cols-2 gap-4 mb-8">
                  <button 
                     ...
                     className={`flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border-2 transition-all shadow-sm ${selectedPaymentMethod === 'CASH' ? 'border-blue-600 bg-blue-50 text-blue-700 ring-4 ring-blue-500/10' : 'border-slate-100 bg-slate-50 text-slate-600 hover:bg-slate-100 hover:border-slate-200'}`}
                  >
                     <Banknote size={36} className="mb-1" />
                     <span className="font-bold text-lg">Cash</span>
                  </button>
                  <button 
                     onClick={() => setSelectedPaymentMethod('KBZ_PAY')}
                     className={`flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border-2 transition-all shadow-sm ${selectedPaymentMethod === 'KBZ_PAY' ? 'border-blue-600 bg-blue-50 text-blue-700 ring-4 ring-blue-500/10' : 'border-slate-100 bg-slate-50 text-slate-600 hover:bg-slate-100 hover:border-slate-200'}`}
                  >
                     <QrCode size={36} className="mb-1" />
                     <span className="font-bold text-lg">KBZ Pay</span>
                  </button>
               </div>

               <div className="flex gap-3">
                  <Button variant="outline" className="flex-1 h-12 text-base font-medium" onClick={() => setPaymentModalOpen(false)}>Cancel</Button>
                  <Button 
                    variant="primary" 
                    className="flex-[2] h-12 bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/20 text-base font-bold" 
        ...
        <div className="fixed inset-0 bg-black z-50 flex flex-col animate-in fade-in duration-300">
           ...
           <div className="p-8 bg-slate-900 text-white text-center">
              ...
              <div className="flex justify-center gap-2 mt-2">
                  ...
                  <Badge variant="neutral" className="bg-white/10 border-0 text-white/70">UPC-A</Badge>
              </div>
           </div>
        </div>
      )}
