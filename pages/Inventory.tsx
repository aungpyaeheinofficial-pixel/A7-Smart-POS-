...
const Inventory = () => {
  ...
  // Retail Categories
  const retailCategories = ['Beverages', 'Snacks', 'Bakery', 'Dairy', 'Pantry', 'Household', 'Personal Care'];
  ...
  const categories = useMemo<string[]>(() => {
    // Combine existing categories in products with the default retail categories
    const existingCats = new Set<string>(products.map(p => p.category));
    retailCategories.forEach(c => existingCats.add(c));
    return ['All', ...Array.from(existingCats)];
  }, [products]);
  ...
  const handleExport = () => {
    ...
    const rows = filteredProducts.map(p => {
      const status = p.stockLevel < p.minStockLevel ? 'Low Stock' : 'In Stock';
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Button variant="outline" className="gap-2 bg-white hidden sm:flex" onClick={handleExport}>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredProducts.map((product) => {
                 const unitDisplay = getUnitDisplay(product.unit || 'PCS');
                 return (
                 ...
                 <form id="productForm" onSubmit={handleSave} className="space-y-6">
                    ...
                    <div className="space-y-4 pt-2">
                         ...
                         <div className="flex items-center gap-6">
                            <div className="w-32 h-32 rounded-lg bg-slate-50 border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden">
                            ...
                            <div className="space-y-2">
                                ...
                                {currentProduct.image && (
                                    <Button type="button" variant="danger" onClick={removeImage} className="text-xs ml-2">
